...
{
    addSpecification("name", "force wave");
    addSpecification("source", "mage");
    addSpecification("description", "The mage conjures a pounding wave of pure magical force that hammers the target.");
    addSpecification("prerequisites", ([
        "/guilds/mage/evocation/root.c": (["type": "research"]),
        "level": (["type": "level", "value": 9])
    ]));
    ...
    addSpecification("command template", "force wave [at ##Target##]");
    addSpecification("event handler", "forceWaveEvent");
    addSpecification("use ability message",
        "##InitiatorName## send##InitiatorReflexive## a wave of force crashing into ##TargetName##.");
    addSpecification("use ability fail message",
        "You failed to send a force wave at ##TargetName##.");
    addSpecification("use ability cooldown message",
        "You must wait before sending another force wave.");
    addSpecification("damage type", "magical");
    addSpecification("damage hit points", ({
        (["probability": 100, "base damage": 20, "range": 13])
    }));
